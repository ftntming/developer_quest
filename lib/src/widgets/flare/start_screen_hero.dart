...
class StartScreenHero extends LeafRenderObjectWidget {
  ...
  @override
  void updateRenderObject(BuildContext context, covariant StartScreenHeroRenderObject renderObject) {
    renderObject
      ..filename = filename
      ..fit = fit
      ..alignment = alignment
  ...
  @override
  void didUnmountRenderObject(covariant StartScreenHeroRenderObject renderObject) {
...
class StartScreenHeroRenderObject extends FlareRenderBox {
  ...
  void paintFlare(Canvas canvas, Mat2D viewTransform) {
    ...
    List<double> stops = <double>[0, (_character.height - offsetTop) / height, 1];

    Offset start = Offset(-1 * _character.origin[0] * _character.width - paddingArtboardSpace,
    ...
    canvas.drawRect(start & Size(_character.width + paddingArtboardSpace * 2, height), paint);
    ...
    if (_crossFade > 0) {
      ...
      canvas.drawRect(
          Offset(-1 * _character.origin[0] * _character.width - paddingArtboardSpace,
                  -1 * _character.origin[1] * _character.height - _character.height) &
              Size(_character.width + paddingArtboardSpace * 2, _character.height * 3),
          darken);
    }
  }
  ...
  Future<void> load() async {
    FlutterActor actor = await loadFlare(AssetFlare(bundle: rootBundle, name: filename));
    if (actor == null) {
      return;
    }
